...
@Repository("judgeMealDao")
public class JudgeMealDao extends AbstractHibernateDao<JudgeMeal> {

	public JudgeMealDao() {
		super();

		setClazz(JudgeMeal.class);
	}

	@Override
	public JudgeMeal getObject(JudgeMeal obj) {
		List<JudgeMeal> objs = getObjects(obj);
	...
	@Override
	public List<JudgeMeal> getObjects(JudgeMeal obj) {
		Example example = Example.create(obj).ignoreCase()
				.enableLike(MatchMode.ANYWHERE);
		return getCurrentSession().createCriteria(JudgeMeal.class).add(example)
				.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
	}

	@Override
	/**单个忽略（或关注）查詢对象字段（非值）*/
	public JudgeMeal getObject(JudgeMeal obj, String excludeField, String type) {
		List<JudgeMeal> objs = getObjects(obj, excludeField, type);
		if (objs != null && objs.size() > 0)
			return objs.get(0);
		else
			return null;
	...
	@Override
	/**集合忽略（或关注）查詢对象字段（非值）*/
	public List<JudgeMeal> getObjects(JudgeMeal obj, String excludeField, String type) {
		Criteria criteria = getCurrentSession().createCriteria(JudgeMeal.class);
	...
	@Override
	public Object[] Search(JudgeMeal entity, Page page,
