...
@Repository("mealDao")
public class MealDao extends AbstractHibernateDao<Meal> {
	...
	@Override
	public Meal getObject(Meal obj) {
		List<Meal> objs = getObjects(obj);
	...
	@Override
	public List<Meal> getObjects(Meal obj) {
		Example example = Example.create(obj).ignoreCase()
				.enableLike(MatchMode.ANYWHERE);
		return getCurrentSession().createCriteria(Meal.class).add(example)
				.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
	}

	@Override
	/**单个忽略（或关注）查詢对象字段（非值）*/
	public Meal getObject(Meal obj, String excludeField, String type) {
		List<Meal> objs = getObjects(obj, excludeField, type);
		if (objs != null && objs.size() > 0)
			return objs.get(0);
		else
			return null;
	...
	@Override
	/**集合忽略（或关注）查詢对象字段（非值）*/
	public List<Meal> getObjects(Meal obj, String excludeField, String type) {
	...
	@Override
	public Object[] Search(Meal entity, Page page,
