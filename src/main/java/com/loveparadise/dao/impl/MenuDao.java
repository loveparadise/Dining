...
@Repository("menuDao")
public class MenuDao extends AbstractHibernateDao<Menu> {
	...
	@Override
	public Menu getObject(Menu obj) {
		List<Menu> objs = getObjects(obj);
	...
	@Override
	public List<Menu> getObjects(Menu obj) {
		Example example = Example.create(obj).ignoreCase()
				.enableLike(MatchMode.ANYWHERE);
		return getCurrentSession().createCriteria(Menu.class).add(example)
				.setResultTransformer(Criteria.DISTINCT_ROOT_ENTITY).list();
	}

	@Override
	/**单个忽略（或关注）查詢对象字段（非值）*/
	public Menu getObject(Menu obj, String excludeField, String type) {
		List<Menu> objs = getObjects(obj, excludeField, type);
		if (objs != null && objs.size() > 0)
			return objs.get(0);
		else
			return null;
	...
	@Override
	/**集合忽略（或关注）查詢对象字段（非值）*/
	public List<Menu> getObjects(Menu obj, String excludeField, String type) {
	...
	@Override
	public Object[] Search(Menu entity, Page page,
