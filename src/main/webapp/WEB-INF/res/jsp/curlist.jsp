					...
					<div class="row">
						...
						<div class="alert alert-info" id="requirement"
							...
							<label for="form-field-select-3">箱规选择:</label> <select
								...
								placeholder="请录入整数" title="没手动录入数字情况下，系统自动运算取值!" />
							&nbsp;&nbsp; <label for="form-field-select-3">箱贴类型:</label> <select
								class="chosen-select" style="width:180px;"
								data-placeholder="箱贴类型" id="pateType" onchange="switchSelect()">
									<option value="Standard">整箱箱贴</option>
									<option value="Remainder">尾箱(或拼装箱)箱贴</option>
									<option value="Mix">配件混款混色箱贴</option>
									<option value="MixOfShoes">鞋类混款混色箱码</option>
<!-- 									<option value="Underpants">内裤箱贴</option> -->
							</select>&nbsp;&nbsp;
							<button type="button" class="btn btn-sm btn-success"
								onclick="check()">
								添加装箱货品 <i class="icon-arrow-right icon-on-right bigger-110"></i>
							</button>
							&nbsp;&nbsp;
					...
					<div class="modal-dialog">
						<div class="modal-content" style="height: 375px;">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal" onclick="cleanUp()";
									aria-hidden="true">&times;</button>
								<h4 class="modal-title" id="myModalLabel">装箱货品明细</h4>
							</div>
							<div class="modal-body" style="height: 300px;overflow:auto;">
								<div>
									<div class="col-xs-7" style="padding-left: 0px;"><input type="text" class="form-control" id="goodCode" placeholder="请录入货品条码，支持扫描枪" onkeydown='if(event.keyCode==13){addItem();}'></div>
									&nbsp; <button class="btn btn-sm btn-success" onclick="addItem();">添加货品</button>&nbsp;&nbsp;<input type="text" id="weight" placeholder="重量录入" style="width: 70px;">
								</div>
								<br/>
								<div class="row">
								<div class="col-xs-12">
							...
							<div class="modal-footer">
								<button type="button" class="btn btn-default" onclick="cleanUp()"
									data-dismiss="modal">关闭</button>
								<button type="button" class="btn btn-primary"
									onclick="validateAndCreate()">生成箱贴</button>
							</div>
						</div>
						<!-- /.modal-content -->
					</div>
						...
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal"
									aria-hidden="true">&times;</button>
								<h4 class="modal-title" id="myModalLabel2">确认生成箱贴</h4>
							</div>
							...
							<div class="modal-footer">
								<button type="button" class="btn btn-default"
									data-dismiss="modal">关闭</button>
								<button type="button" class="btn btn-primary" id="create2"
									onclick="showAddGoodWin();">生成箱贴</button>
							</div>
						</div>
						<!-- /.modal-content -->
					</div>
	...
	<script type="text/javascript">
		...
		var rowIndex=1;//所添加货品的index
		var existGoodCode="";//已经添加货品编码的字符串
		var addNum=0;//已添加货品的数量
		
		var eanGoodCodeStr="";//所选数据行的货物编码
		var map = {};//记录选中数据行的‘剩余量’及对应的‘单品条码’
		...
		function printAgain()
		{
			if($("#FILEtoPrint").length==0)
			{
				...
				alertMsg($("#alertMsg"),"alert alert-danger alert-dismissable","错误","还未生成过箱贴!");
				return false;
			}
			
			setTimeout(function(){$("#print_button").click();}, 10);
		}
		function cleanUp()
		{
			$("#myModalLabel").html("装箱货品明细");
		...
		function addItem(){
			...
			if(eanGoodCodeStr.indexOf(goodCode+",")<0)
			{
				playError();
				alert("输入的货品编码不在所选数据行中！！！");
				$("#goodCode").focus();
				$("#goodCode").val("");
				return false;
			}
			
			if($("#packNum").val()>0&&$("#packNum").val()==addNum)
			{
				playError();
				alert("已达最大包装量，不能再添加货品！！！");
				$("#goodCode").focus();
				$("#goodCode").val("");
				return false;
			}else if($("#packNum").val().length==0&&zcarvol==addNum)
			{
				playError();
				alert("已达最大包装量，不能再添加货品！！！");
				$("#goodCode").focus();
			...
			{
				playError();
				alert(goodCode+"对应货品的剩余量已经为0，不再添加相应的货品！");
				$("#goodCode").focus();
				$("#goodCode").val("");
				return false;
			}
// 			if(existGoodCode.indexOf($("#goodCode").val()+",")>=0)
// 			{
// 				alert("该货品已存在该包装中！！！");
// 				$("#goodCode").val("");
// 				return false;
// 			}else
			{
				existGoodCode=existGoodCode+$.trim($("#goodCode").val())+",";
				var htm="<tr id='row"+rowIndex+"'><td id='td"+rowIndex+"'>"+$("#goodCode").val()+"</td><td><div class='visible-md visible-lg hidden-sm hidden-xs btn-group'>"+
					"<button class='btn btn-xs btn-danger' onclick=deleteItem('"+rowIndex+"')><i class='icon-trash bigger-120'></i></button></div></td></tr>";
				$("#tbody").append(htm);
				rowIndex++;
				addNum++;
				$("#myModalLabel").html("已添加"+addNum+"件货品！！！");
			}
			$("#goodCode").val("");
			$("#goodCode").focus();
		...
		function deleteItem(obj){
			var rowId="#row"+obj;
			var tdId="#td"+obj;
			existGoodCode=existGoodCode.replace($(tdId).html()+",","");
			$(rowId).html("");
			addNum--;
			$("#myModalLabel").html("已添加"+addNum+"件货品！！！");
		}
		...
		function check() {
			...
			{
				//显示提示信息参数分别表示：信息要显示的DIV对象，显示样式，消息标题，消息内容
				alertMsg($("#alertMsg"),"alert alert-danger alert-dismissable","错误","现场打印只支持‘谷歌’浏览器，请安装‘谷歌’浏览器，再登录系统使用！");
				return false;
			}
			
			var zcarvolFlag=false;//有数据行没有默认箱规标示
			...
			var skuweightFlag=false;//是否维护sku重量
			...
			for(var i=0;i<rowIds.length;i++)
			{
				...
				if($(grid_selector).jqGrid('getRowData', rowIds[i]).eanGoodCode!="")
				{
					eanGoodCodeStr=eanGoodCodeStr+$(grid_selector).jqGrid('getRowData', rowIds[i]).eanGoodCode+",";
				}
				if(i==0)
				{
				...
				map[$(grid_selector).jqGrid('getRowData', rowIds[i]).eanGoodCode]=$(grid_selector).jqGrid('getRowData', rowIds[i]).allowance;
				...
				if($(grid_selector).jqGrid('getRowData', rowIds[i]).skuweight=="" && $(grid_selector).jqGrid('getRowData', rowIds[i]).typeName!="鞋类")//数据行没维护sku重量
				{
					skuweightFlag=true;
				}
				...
				if($(grid_selector).jqGrid('getRowData', rowIds[i]).zcarvol=="")//数据行包含 不自带箱规的 数据行
				{
					zcarvolFlag=true;
				}
...
// 				}
			...
			}else 
			{
				...
				{
					alertMsg($("#alertMsg"),"alert alert-danger alert-dismissable","错误","所选择数据行有数据没有默认箱规，请输入装箱量!");
					return false;
				}
			...
			if($("#pateType").val()=="Remainder")//如果是尾箱
			{
				...
				if(skuweightFlag)
				{
					alertMsg($("#alertMsg"),"alert alert-danger alert-dismissable","错误","请维护sku重量！！！");
					return false;
				}
			...
			if(boxNum<=0)
			{
				alertMsg($("#alertMsg"),"alert alert-danger alert-dismissable","错误","所选择数据行的总余量小于装箱量！！");
				return false;
			}
			
			if(eanGoodCodeStr=="")
			{
				alertMsg($("#alertMsg"),"alert alert-danger alert-dismissable","错误","所选择数据行没有‘单品条码’，无法将货品与数据行进行关联，请联系管理员！！");
				return false;
			}
			closeAlertMsg();
			showModal();		
		...
		function validateAndCreate() {
			...
			if(pateType == "MixOfShoes")
				pateType = "Mix";
			$.ajax({
				url : basePath + "print/createCurBoxPates",
				method : "post",
				data : {
					Ids : rowIds,
					boxId : $("#boxes").val(),
				...
				success : function(data) {
					$.each(data, function(id, item) {
						if(id=="message")
						{
							if (item == "success") {
								boxNum--;
								$("#myModalLabel").html("<span style='color:red'>已生成！！！</span>");
								rowIndex=1;
								existGoodCode="";
								addNum=0;
								$("#tbody").html("");
								...
								$("#myModalLabel").html("<span style='color:red'>生成失败！！！箱规不符合要求或对应数据行在作业中！！！</span>");
								$("#confirmModal").modal('toggle');
								$("#goodCode").focus();
							}
						}else if(id=="fileName")
				...
				error : function(jqXHR, textStatus, errorThrown) {
					$("#myModalLabel").html("<span style='color:red'>访问服务器错误，请重新登录系统，如果还是报该错误，请联系管理员！！！！！</span>");
					$("#confirmModal").modal({backdrop: 'static', keyboard: false});
					$("#goodCode").focus();
				}
			});
		...
		function getBoxes() {
			$.ajax({
				...
				error : function errorCallback(xmlhttpRequest, textStatus,
						errorThrown) {
		...
		jQuery(function($) {
			...
			jQuery("#breadcrumbs").resize(
					function() {
						...
						jQuery("#requirement").width(
								jQuery("#breadcrumbs").width() - 50);
					});
					...
					.jqGrid(
							{
								...
								onSelectRow : function(rowId, status, e) {//勾选时要执行的方法
									//var MATNR=$(grid_selector).jqGrid('getRowData', rowId).MATNR;
									weight=$(grid_selector).jqGrid(
											'getRowData', rowId).weight;
									$("#weight").val(weight);
									if (status) {
										if($("#pateType").val()=="Standard")
										{	
								...
								onPaging : function() {//翻页时要执行的方法
									$(grid_selector).jqGrid('setGridParam', {
										_search : false
									});//如果是翻页，则置标识，该操作不是新的查询动作
								},
								loadComplete : function() {//表格加载完毕要执行的方法
									$("#cb_grid-table").attr("disabled","disabled");
									var total = $(grid_selector).jqGrid(
											'getGridParam', 'lastpage');//获取总页数
									var records = $(grid_selector).jqGrid(
											'getGridParam', 'records');//获取总记录数
									var postData = {
										totalPage : total,
										recordCount : records
									};
									...
									//使表格前面的操作按钮在右边显示
									$(grid_selector).closest(
											"div.ui-jqgrid-view").children(
											"div.ui-jqgrid-titlebar").css(
											"text-align", "right").children(
											"span.ui-jqgrid-title").css(
											"float", "right");
								},
