	...
	<div class="main-container" id="main-container">
		<div class="main-container-inner">
						...
						<div class="col-xs-12">
							...
							<div id="frameDialog">
								<div style="text-align:center;width:auto">
									...
									src="<%=basePath%>order/editiframe?id=-1"
			...
			$(function() {
				...
				$("#frameDialog").attr("width", bw);
				$("#frameDialog").attr("height", bh);
				$("#iframe").attr("width", bw);
				$("#iframe").attr("height", bh);
			});
			...
			function showEditFrame() {
				...
				$(".nav nav-list ul li").removeClass("submenu");
				$("#frameDialog").show();
			}

			function hideFrame() {
				$("#fullbg,#frameDialog").hide();
				$(".nav nav-list ul li").addClass("submenu");
				window.frames[0].location.href = window.frames[0].location.href
						+ '?random=' + Math.random();
			}

			function editIframe() {
				var src = basePath + "order/editiframe?" + "id=" + rowIds;
				$("#iframe").attr("src", src);
				showEditFrame();
			}
			...
			jQuery(function($) {
				//注意每个页面都要在“jQuery(function($) {”加入如下resize function
				 ...
				 var btnStr="";
				 btnStr="<button class='btn btn-xs btn-yellow' onclick='editIframe()'>编辑</button>";
				jQuery(grid_selector).jqGrid({
					url:basePath+"order/editOrderDetails",
					datatype: "json",
					height:350,
					colNames:[' ',
					          'ID',
							  ...
							  colModel:[
										...
										{name:'myac',index:'', width:50, fixed:true, sortable:false, resize:false,search:false,hidden:true,
											formatter:'actions', 
											formatoptions:{ 
												keys:true,
										...
										{name:'EBELN',index:'EBELN', 
											editable:true,
											//formatter:"showlink",
					                        //formatoptions:{baseLinkUrl:"<%=basePath%>order/editiframe",idName:'id'},
										...
										{name:'EBELP',index:'EBELP', editable:true,searchoptions: {sopt: ['cn']}},
										{name:'TXZ01',index:'TXZ01', editable:true,searchoptions: {sopt: ['cn']}},
										{name:'ZTEXT13',index:'ZTEXT13', editable:true,searchoptions: {sopt: ['cn']}},
										{name:'ETENR',index:'ETENR', editable:true,searchoptions: {sopt: ['cn']}}
									], 
					...
					editurl: $path_base+"/dummy.html",//新增，修改，删除所提交的路径，根据不动页面设置 不同值，有可能要动态来改变，达到操作需求
