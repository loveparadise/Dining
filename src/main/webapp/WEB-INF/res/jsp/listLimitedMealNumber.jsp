				 ...
				 if(typeStr=="")
					{
						 ...
						 array.push('${mealType}'+":"+'${mealType.getText()}'+";"); //js中可以使用此标签，将EL表达式中的值push到数组中  
						 </c:forEach>  
						 for(var i=0;i<array.length;i++)  
						 {  
							 mealTypeStr=mealTypeStr+array[i];
						 }
						 editMealTypeStr=mealTypeStr.substring(0,mealTypeStr.length-1);
						 mealTypeStr="-99:全部;"+mealTypeStr.substring(0,mealTypeStr.length-1);
						 
						 array = new Array();  
						 <c:forEach items="${diningRooms}" var="diningRoom">  
						 array.push('${diningRoom.id}'+":"+'${diningRoom.name}'+";"); //js中可以使用此标签，将EL表达式中的值push到数组中  
						 </c:forEach>  
						 for(var i=0;i<array.length;i++)  
						 {  
							 diningRoomStr=diningRoomStr+array[i];
						 }
						 editDiningRoomStr=diningRoomStr.substring(0,diningRoomStr.length-1);
						 diningRoomStr="-99:全部;"+diningRoomStr.substring(0,diningRoomStr.length-1);
					}
				 
					
				
				jQuery(grid_selector).jqGrid({
					url:basePath+"common/listLimitedMealNumber",
					datatype: "json",
					height:350,
					colNames:['ID','餐厅ID','餐厅名称','可用餐人数','限餐时段','限餐日期','状态','备注','更新日期','更新人'],
					colModel:[
// 						{name:'myac',index:'', width:80, fixed:true, sortable:false, resize:false,search:false,hidden:false,
// 							formatter:'actions', 
// 							formatoptions:{ 
// 								keys:true,
// 								delOptions:{recreateForm: true, beforeShowForm:beforeDeleteCallback,afterShowForm:afterDeleteCallback},
// 							}
// 						},
						{name:'id',index:'id',  sorttype:"int", editable:false,hidden:true},
						{name:'diningRoom_id',index:'diningRoom_id',  sorttype:"int", editable:false,hidden:true},
						{name:'diningRoom_name',index:'diningRoom_name',sortable:false,
							stype:"select",
							searchoptions:{sopt:["eq"],value:diningRoomStr},
							editable:true,
							edittype:"select",
							editoptions:{value:editDiningRoomStr}
						},
						{name:'number',index:'number',editable:true,searchoptions: {sopt: ['eq']}},
						{name:'mealType',index:'mealType',sortable:false,
							stype:"select",
							searchoptions:{sopt:["eq"],value:mealTypeStr},
							editable:true,
							edittype:"select",
							editoptions:{value:editMealTypeStr}
						},
						{name:'dayDate',index:'dayDate', sorttype:"date", editable:true,searchoptions: {dataInit:datePick,sopt: ['eq','le','ge'],attr:{title:"选择日期"}},formatter:'date',
					...
					editurl: basePath+"common/saveOrUpdateLimitedMealNumber",//新增，修改，删除所提交的路径，根据不动页面设置 不同值，有可能要动态来改变，达到操作需求
					autowidth: true			
				});
				...
				jQuery(grid_selector).jqGrid('navGrid',pager_selector,
					...
					},
					{
						...
						beforeShowForm : function(e) {
							...
							$('#dayDate').attr("data-date-format","yyyy-mm-dd");
