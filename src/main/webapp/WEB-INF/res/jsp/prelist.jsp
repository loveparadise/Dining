...
// 				}
			...
			function check()
			{
				...
				var allBoxFlag=true;//所选择数据行合部以箱为单位
...
// 					}
					...
					if($(grid_selector).jqGrid('getRowData', rowIds[i]).zcarvol==""&&$(grid_selector).jqGrid('getRowData', rowIds[i]).packNum=="")//数据行包含 导入装箱量为空的数据行
					{
						zcarvolFlag=true;
					}
					
					if($(grid_selector).jqGrid('getRowData', rowIds[i]).selfBoxName==""&&$(grid_selector).jqGrid('getRowData', rowIds[i]).boxName=="")//数据行包含 导入箱规为空的数据行
					{
...
// 					}
				...
				if($("#pateType").val()=="Mix")//如果是直配
				{
					...
					if(matnrNum>12)
					{
						alertMsg($("#alertMsg"),"alert alert-danger alert-dismissable","错误","直配混款混色箱只能包含12个货号！！！");
						return false;
					}else if(sizeNum1>1||sizeNum2>1||sizeNum3>1)
					{
						alertMsg($("#alertMsg"),"alert alert-danger alert-dismissable","错误","直配混款混色箱只能包含12个货号，各货号最多只能有1个尺码！！！");
						return false;
					}
				}
						...
						$.each(data, function(id, item) {
							if (item == "success") {
								...
								$(grid_selector).trigger("reloadGrid");
									...
									alertMsg($("#alertMsg"),"alert alert-danger alert-dismissable","失败","‘剩余量’小于装箱量、或服装和配件的SKU重量未维护、或使用的箱规不符合要求都会造成生成失败！或有数据正在作业，请稍后再试！");//现阶段只支持鞋、服、配的 整箱箱码及鞋类尾箱的箱码！
// 									$("input[class='cbox']").removeAttr("checked");
